...
namespace cl {
  ...
  uint clHierarchicalStructure_T<NODE>::GetNextAvaliableNodeId() {
  ...
  template<typename NODE>
  NODE* clHierarchicalStructure_T<NODE>::GetNode(uint id) {
  ...
  template<typename NODE>
  NODE* clHierarchicalStructure_T<NODE>::DeleteNode(uint id) {
  ...
  template<typename NODE>
  bool clHierarchicalStructure_T<NODE>::DeleteDecendantNodes(uint id) {
	 NODE* tmpNode = GetNode(id);
  ...
  template<typename NODE>
  NODE* clHierarchicalStructure_T<NODE>::RemoveNode(uint id) {
	 NODE* tmpNode = GetNode(id);
	 if (tmpNode == nullptr)return nullptr;
	 else {
		RemoveRelation(tmpNode);
  ...
  template<typename NODE>
  uint clHierarchicalStructure_T<NODE>::GetNumChildren(NODE* node)const {
	 uint sum = 0;
	 NODE* tmpNode = node->m_pFirstChildNode;
  ...
  template<typename NODE>
  uint clHierarchicalStructure_T<NODE>::GetNumChildren()const {
	 return GetNumChildren(m_pRootNode);
  }

  template<typename NODE>
  uint clHierarchicalStructure_T<NODE>::GetDepth(NODE* node) {
	 uint depth(0);
  ...
  void clHierarchicalStructure_T<NODE>::Print() {
	 ...
	 while (vec.size() != 0) {
		...
		uint w = (2 + GetDepth(tmpNode))*perDepthWidth;
