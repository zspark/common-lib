...
namespace clCon {
	...
	void clConsole::SetDefaultColor(clConForeColor::color defaultColor, clConBackColor::color backgroundColor) {
		m_conDefaultColor = defaultColor | backgroundColor;
	}

	void clConsole::SetInfoColor(clConForeColor::color defaultColor, clConBackColor::color backgroundColor) {
		m_conInfoColor = defaultColor | backgroundColor;
	}

	void clConsole::SetWarningColor(clConForeColor::color defaultColor, clConBackColor::color backgroundColor) {
		m_conWarningColor = defaultColor | backgroundColor;
	}

	void clConsole::SetErrorColor(clConForeColor::color defaultColor, clConBackColor::color backgroundColor) {
		m_conErrorColor = defaultColor | backgroundColor;
	}

	void clConsole::SetInputColor(clConForeColor::color defaultColor, clConBackColor::color backgroundColor) {
	...
	clConsole::clConsole() {
		...
		m_conDefaultColor = clConForeColor::DARKGRAY | backColor;
		m_conInfoColor = clConForeColor::GREEN | backColor;
		m_conWarningColor = clConForeColor::YELLOW | backColor;
		m_conErrorColor = clConForeColor::RED | backColor;
		m_conInputColor = clConForeColor::BLUE | clConBackColor::WHITE;

		/*
			COORD coord;coord.X=80; coord.Y=10;
			SetConsoleScreenBufferSize(s_handler,coord);
			coord.X=0; coord.Y=0;
			DWORD att=clConForeColor::WHITE|clConBackColor::DARKBLUE;
