...
namespace cl {
  ...
  public:
	 clHierarchicalStructureNode_T(uint id) :m_id(id) {};
	 ...
	 //One cant change id of a exist node;void SetId(uint id) { m_id=id; }
  ...
  public:
	 ...
	 uint GetNumChildren(NODE* node)const;
  ...
  private:
	 ...
	 uint GetNextAvaliableNodeId();

	 //delete node;
	 bool DeleteDecendantNodes(NODE* node);

  private:
	 uint m_uNextAvaliableId = 1;
